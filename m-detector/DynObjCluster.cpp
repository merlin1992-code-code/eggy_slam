                                   ...
                                   const Eigen::Vector3d odom_pos_in)
{
  ...
  bbox_t bbox_high;
  ClusterAndTrack(dyn_tag, cloud_clean_ptr, header, bbox_high, delta_t,
                  raw_point);
    ...
    std::unordered_set<int> &used_map_set)
{
       ...
       it++, j++)
  {
         ...
         z_size > Voxel_revolusion + 0.001f))
    {
      ...
      center.header.seq = current_header.seq;
      center.header.stamp = current_header.stamp;
      center.header.frame_id = current_header.frame_id;
