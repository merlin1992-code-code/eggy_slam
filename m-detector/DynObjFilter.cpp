...
void DynObjFilter::init(NodeHandle &nh)
{
  nh.param<double>("dyn_obj/buffer_delay", buffer_delay, 0.1);
  nh.param<int>("dyn_obj/buffer_size", buffer_size, 700000);
  nh.param<int>("dyn_obj/points_num_perframe", points_num_perframe, 300000);
  nh.param<double>("dyn_obj/depth_map_dur", depth_map_dur, 0.2);
  nh.param<int>("dyn_obj/max_depth_map_num", max_depth_map_num, 5);
  nh.param<int>("dyn_obj/max_pixel_points", max_pixel_points, 5);
  nh.param<double>("dyn_obj/frame_dur", frame_dur, 0.1);
  nh.param<int>("dyn_obj/dataset", dataset, 2);
  nh.param<float>("dyn_obj/self_x_f", self_x_f, 0.5f);
  ...
  nh.param<float>("dyn_obj/enter_min_thr1", enter_min_thr1, 1.0f);
  nh.param<float>("dyn_obj/enter_max_thr1", enter_max_thr1, 0.3f);
  nh.param<float>("dyn_obj/map_cons_depth_thr1", map_cons_depth_thr1, 0.3f);
  ...
  nh.param<float>("dyn_obj/map_cons_hor_dis1", map_cons_hor_dis1, 0.15f);
  nh.param<float>("dyn_obj/map_cons_ver_dis1", map_cons_ver_dis1, 0.15f);
  nh.param<float>("dyn_obj/depth_cons_depth_thr1", depth_cons_depth_thr1, 0.5f);
  nh.param<float>("dyn_obj/depth_cons_depth_max_thr1",
                  depth_cons_depth_max_thr1, 1.0f);
  nh.param<float>("dyn_obj/depth_cons_hor_thr1", depth_cons_hor_thr1, 0.01f);
  nh.param<float>("dyn_obj/depth_cons_ver_thr1", depth_cons_ver_thr1, 0.01f);
  nh.param<float>("dyn_obj/enlarge_z_thr1", enlarge_z_thr1, -2.5f);
  nh.param<float>("dyn_obj/enlarge_angle", enlarge_angle, 2.0f);
  nh.param<float>("dyn_obj/enlarge_depth", enlarge_depth, 3.0f);
  nh.param<int>("dyn_obj/occluded_map_thr1", occluded_map_thr1, 2);
  nh.param<bool>("dyn_obj/case1_interp_en", case1_interp_en, true);
  ...
  nh.param<float>("dyn_obj/acc_thr2", acc_thr2, 7.0f);
  nh.param<float>("dyn_obj/map_cons_depth_thr2", map_cons_depth_thr2, 0.2f);
  nh.param<float>("dyn_obj/map_cons_hor_thr2", map_cons_hor_thr2, 0.01f);
  ...
  nh.param<float>("dyn_obj/occ_hor_thr2", occ_hor_thr2, 0.01f);
  nh.param<float>("dyn_obj/occ_ver_thr2", occ_ver_thr2, 0.01f);
  nh.param<float>("dyn_obj/depth_cons_depth_thr2", depth_cons_depth_thr2, 0.1f);
  nh.param<float>("dyn_obj/depth_cons_depth_max_thr2",
                  depth_cons_depth_max_thr2, 0.5f);
  nh.param<float>("dyn_obj/depth_cons_hor_thr2", depth_cons_hor_thr2, 0.01f);
  nh.param<float>("dyn_obj/depth_cons_ver_thr2", depth_cons_ver_thr2, 0.01f);
  nh.param<float>("dyn_obj/k_depth2", k_depth2, 0.005f);
  nh.param<int>("dyn_obj/occluded_times_thr2", occluded_times_thr2, 2);
  ...
  nh.param<float>("dyn_obj/acc_thr3", acc_thr3, 15.0f);
  nh.param<float>("dyn_obj/map_cons_depth_thr3", map_cons_depth_thr3, 0.2f);
  nh.param<float>("dyn_obj/map_cons_hor_thr3", map_cons_hor_thr3, 0.01f);
  nh.param<float>("dyn_obj/map_cons_ver_thr3", map_cons_ver_thr3, 0.006f);
  nh.param<float>("dyn_obj/occ_depth_thr3", occ_depth_thr3, 0.15f);
  nh.param<float>("dyn_obj/occ_hor_thr3", occ_hor_thr3, 0.01f);
  nh.param<float>("dyn_obj/occ_ver_thr3", occ_ver_thr3, 0.01f);
  nh.param<float>("dyn_obj/depth_cons_depth_thr3", depth_cons_depth_thr3, 0.3f);
  nh.param<float>("dyn_obj/depth_cons_depth_max_thr3",
                  depth_cons_depth_max_thr3, 1.0f);
  nh.param<float>("dyn_obj/depth_cons_hor_thr3", depth_cons_hor_thr3, 0.01f);
  nh.param<float>("dyn_obj/depth_cons_ver_thr3", depth_cons_ver_thr3, 0.01f);
  nh.param<float>("dyn_obj/k_depth3", k_depth3, 0.005f);
  nh.param<int>("dyn_obj/occluding_times_thr3", occluding_times_thr3, 2);
  ...
  nh.param<float>("dyn_obj/interp_hor_thr", interp_hor_thr, 0.015f);
  nh.param<float>("dyn_obj/interp_ver_thr", interp_ver_thr, 0.01f);
  nh.param<float>("dyn_obj/interp_thr1", interp_thr1, 0.5f);
  nh.param<float>("dyn_obj/interp_static_max", interp_static_max, 10010.0f);
  nh.param<float>("dyn_obj/interp_start_depth1", interp_start_depth1, 10.0f);
  nh.param<float>("dyn_obj/interp_kp1", interp_kp1, 0.1f);
  nh.param<float>("dyn_obj/interp_kd1", interp_kd1, 0.5f);
  nh.param<float>("dyn_obj/interp_thr2", interp_thr2, 0.25f);
  nh.param<float>("dyn_obj/interp_thr3", interp_thr3, 0.05f);
  nh.param<bool>("dyn_obj/dyn_filter_en", dyn_filter_en, true);
  nh.param<bool>("dyn_obj/debug_publish", debug_en, true);
  nh.param<int>("dyn_obj/laserCloudSteadObj_accu_limit",
  ...
  nh.param<bool>("dyn_obj/cluster_future", cluster_future, true);
  nh.param<int>("dyn_obj/cluster_extend_pixel", Cluster.cluster_extend_pixel,
                2);
  nh.param<int>("dyn_obj/cluster_min_pixel_number",
                Cluster.cluster_min_pixel_number, 3);
  nh.param<float>("dyn_obj/cluster_thrustable_thresold",
                  Cluster.thrustable_thresold, 0.3f);
  nh.param<float>("dyn_obj/cluster_Voxel_revolusion", Cluster.Voxel_revolusion,
                  0.3f);
  nh.param<bool>("dyn_obj/cluster_debug_en", Cluster.debug_en, false);
  nh.param<string>("dyn_obj/cluster_out_file", Cluster.out_file, "");
  nh.param<float>("dyn_obj/hor_resolution_max", ver_resolution_max, 0.01f);  // 0.01f
  nh.param<float>("dyn_obj/ver_resolution_max", hor_resolution_max, 0.007f); // 0.005f
  nh.param<float>("dyn_obj/buffer_dur", buffer_dur, 0.1f);
  nh.param<int>("dyn_obj/point_index", point_index, -1);
  ...
  pixel_fov_up =
      floor((fov_up / 180.0 * PI_MATH + 0.5 * PI_MATH) / ver_resolution_max);
  pixel_fov_down =
      floor((fov_down / 180.0 * PI_MATH + 0.5 * PI_MATH) / ver_resolution_max);
  pixel_fov_cut =
      floor((fov_cut / 180.0 * PI_MATH + 0.5 * PI_MATH) / ver_resolution_max);
  pixel_fov_left =
      floor((fov_left / 180.0 * PI_MATH + PI_MATH) / hor_resolution_max);
  pixel_fov_right =
      floor((fov_right / 180.0 * PI_MATH + PI_MATH) / hor_resolution_max);
  max_pointers_num =
      round((max_depth_map_num * depth_map_dur + buffer_delay) / frame_dur) + 1;
                          ...
                          const double &scan_end_time)
{
  ...
  time_search = time_research = time_search_0 = time_build = time_total =
      time_other0 = 0.0;
    ...
    for (int i = 0; i < size; i++)
    {
      ...
      case CASE1:
        if (dyn_tag_cluster[i] == 0)
        {
          ...
          PointType po_tmp = po;
          po_tmp.x = points[i]->local(0);
          po_tmp.y = points[i]->local(1);
          po_tmp.z = points[i]->local(2);
          laserCloudSteadObj_clus->push_back(points[i]->point);
        ...
        else // case1
        {
          ...
          po.normal_y = points[i]->is_occu_times;
          po.normal_z = points[i]->occu_times;
          laserCloudDynObj_clus->push_back(points[i]->point);
  ...
  double total_test1 = 0, total_test2 = 0, total_test3 = 0, total_proj = 0,
         total_occ = 0, total_map = 0;
  for (int i = 0; i < size; i++)
  {
  ...
  if (time_breakdown_file != "")
  {
    time_breakdown_out << total_test1 << " " << total_test2 << " "
                       << total_test3 << " " << total_proj << " " << total_occ
                       << " " << total_map << endl;
  }
  frame_num_for_rec++;
  ...
  time_ind++;
  time_total_avr =
      time_total_avr * (time_ind - 1) / time_ind + time_total / time_ind;
}

void DynObjFilter::Points2Buffer(vector<point_soph *> &points,
  ...
  for (int k = 0; k < len; k++)
  {
    ...
    if ((cur_time - point->time) >= buffer_delay - frame_dur / 2.0)
    {
      if (depth_map_list.size() == 0)
      {
        if (depth_map_list.size() < max_depth_map_num)
        {
          ...
          DepthMap::Ptr new_map_pointer(
              new DepthMap(point->rot, point->transl, point->time, map_index));
               ...
               depth_map_dur - frame_dur / 2.0)
      {
        ...
        if (depth_map_list.size() == max_depth_map_num)
        {
          depth_map_list.front()->Reset(point->rot, point->transl, point->time,
                                        map_index);
      ...
      switch (point->dyn)
      {
        ...
        case STATIC:
          SphericalProjection(*point, depth_map_list.back()->map_index,
                              depth_map_list.back()->project_R,
                              depth_map_list.back()->project_T, *point);
        if (depth_map_list.back()->depth_map[point->position].size() <
            max_pixel_points)
        {
          depth_map_list.back()->depth_map[point->position].push_back(point);
          if (point->vec(2) >
              depth_map_list.back()->max_depth_all[point->position])
          {
            depth_map_list.back()->max_depth_all[point->position] =
                point->vec(2);
            depth_map_list.back()->max_depth_index_all[point->position] =
                depth_map_list.back()->depth_map[point->position].size() - 1;
          }
          if (point->vec(2) <
                  depth_map_list.back()->min_depth_all[point->position] ||
          ...
          if (point->vec(2) <
                  depth_map_list.back()->min_depth_static[point->position] ||
              depth_map_list.back()->min_depth_static[point->position] <
                  10E-5)
          {
            depth_map_list.back()->min_depth_static[point->position] =
          ...
          if (point->vec(2) >
              depth_map_list.back()->max_depth_static[point->position])
          {
            depth_map_list.back()->max_depth_static[point->position] =
                point->vec(2);
          }
...
void DynObjFilter::SphericalProjection(point_soph &p, int depth_index,
...
void DynObjFilter::CheckNeighbor(const point_soph &p, const DepthMap &map_info,
...
bool DynObjFilter::Case1(point_soph &p)
{
  ...
  {
    SphericalProjection(p, depth_map_list[i]->map_index,
                        depth_map_list[i]->project_R,
                        depth_map_list[i]->project_T, p);
...
bool DynObjFilter::Case1Enter(const point_soph &p, const DepthMap &map_info)
{
  ...
  float cur_min =
      max(cutoff_value, k_depth_min_thr1 * (p.vec(2) - d_depth_min_thr1)) +
      enter_min_thr1;
  float cur_max =
      max(cutoff_value, k_depth_max_thr1 * (p.vec(2) - d_depth_max_thr1)) +
      enter_max_thr1;
  ...
  if (p.vec(2) < min_depth - cur_max ||
      ...
      (stop_object_detect && min_depth < 10E-5 && max_depth < 10E-5 &&
       map_info.depth_map[p.position].size() > 0 &&
       p.vec(2) < map_info.max_depth_all[p.position] + 1.0))
  {
    case1_num++;
    return true;
  }
...
{
  float hor_half =
      max(map_cons_hor_dis1 / (max(p.vec(2), blind_dis)), map_cons_hor_thr1);
  float ver_half =
      max(map_cons_ver_dis1 / (max(p.vec(2), blind_dis)), map_cons_ver_thr1);
                 ...
                 p.local(1) > self_y_l || p.local(1) < self_y_r))
  {
    ...
    if (p.vec(2) > interp_start_depth1)
      cur_interp +=
          ((p.vec(2) - interp_start_depth1) * interp_kp1 + interp_kd1);
    if (dataset == 0)
    {
                                             ...
                                             const DepthMap2D &depth_map)
{
  if (fabs(p.last_depth_interps.at(
          map_index - depth_map_list.front()->map_index)) > 10E-4)
  {
    float depth_cal =
        p.last_depth_interps.at(map_index - depth_map_list.front()->map_index);
    return depth_cal;
  }
  V3F p_1 = V3F::Zero();
  V3F p_2 = V3F::Zero();
  ...
  for (int ind_hor = -interp_hor_num; ind_hor <= interp_hor_num; ind_hor++)
  {
    for (int ind_ver = -interp_ver_num; ind_ver <= interp_ver_num; ind_ver++)
    {
      ...
      for (int j = 0; j < points_in_pixel.size(); j++)
      {
        ...
        if (fabs(hor_minus) < interp_hor_thr &&
            fabs(ver_minus) < interp_ver_thr)
        {
          all_num++;
          if (point->dyn == STATIC)
          {
          ...
          if ((point->vec(2) - p.vec(2)) <= interp_static_max &&
              (p.vec(2) - point->vec(2)) < 5.0)
          {
            no_bg_num++;
          }
          if (point->dyn == STATIC)
...
bool DynObjFilter::Case2(point_soph &p)
{
  ...
  SphericalProjection(p, depth_map_list[first_i]->map_index,
                      depth_map_list[first_i]->project_R,
                      depth_map_list[first_i]->project_T, p_spherical);
  ...
  {
    if (!Case2MapConsistencyCheck(p_spherical, *depth_map_list[first_i],
                                  case2_interp_en))
    {
      double ti = 0;
      float vi = 0;
      ...
      for (int ind_hor = -occ_hor_num2; ind_hor <= occ_hor_num2; ind_hor++)
      {
        for (int ind_ver = -occ_ver_num2; ind_ver <= occ_ver_num2; ind_ver++)
        {
          ...
          if (pos_new < 0 || pos_new >= MAX_2D_N)
            continue;
          const vector<point_soph *> &points_in_pixel =
              depth_map_list[first_i]->depth_map[pos_new];
          if (depth_map_list[first_i]->min_depth_all[pos_new] >
              p_spherical.vec(2))
          {
            continue;
          }
          for (int k = 0; k < points_in_pixel.size() && map_cons; k++)
          {
            ...
            if (Case2IsOccluded(p_spherical, *p_occ) &&
                Case2DepthConsistencyCheck(*p_occ, *depth_map_list[first_i]))
            {
              cur_occ_times = 1;
              ...
              while (i >= 0)
              {
                if (p1.occu_index[0] == -1 ||
                    p1.occu_index[0] < depth_map_list.front()->map_index)
                {
                  SphericalProjection(p1, depth_map_list[i]->map_index,
                                      depth_map_list[i]->project_R,
                                      depth_map_list[i]->project_T, p1);
                ...
                SphericalProjection(p, depth_map_list[i]->map_index,
                                    depth_map_list[i]->project_R,
                                    depth_map_list[i]->project_T, p);
                if (Case2MapConsistencyCheck(p, *depth_map_list[i],
                                             case2_interp_en))
                {
                  map_cons = false;
                  break;
                ...
                if (Case2IsOccluded(p, *p2) &&
                    Case2DepthConsistencyCheck(*p2, *depth_map_list[i]) &&
...
// '''
//   Case2Enter 用于辅助判断某个点（或物体）是新进入视野的动态障碍物。
//   它的主要思想是：如果当前点在历史深度图对应像素格中不是最近的那个，且比历史最近点“更远”，并且距离超过一定阈值，则判定为‘进入’（即可能是新出现的动态目标）;
//   主要流程：
//   仅对当前判为静态的点(p.dyn == STATIC)才做 Case2Enter 检查;
//   计算该点在历史深度图对应像素格中的最大深度(max_depth);
//   如果当前点距离比历史最大深度还远,并且大于阈值(depth_thr2_final),就认为它是“新出现的/进入的点”，返回 true;
//   物理意义简述：
//   当前点比历史所有点都“更远”，说明这个物体可能刚刚进入视野，且不是静态背景的一部分;
//   这种判据有助于检测突然出现的物体（如突然驶入的汽车、行人等）;
// '''
bool DynObjFilter::Case2Enter(point_soph &p, const DepthMap &map_info)
{
  ...
  float max_depth = 0;
  float depth_thr2_final =
      max(cutoff_value, k_depth_max_thr2 * (p.vec(2) - d_depth_max_thr2)) +
      occ_depth_thr2;
  if (map_info.depth_map[p.position].size() > 0)
  {
...
//   // --- 加入越界保护 ---
//   if (p.position < 0 || p.position >= map_info.depth_map.size()) {
//     return false;
//   }
//   if (map_info.max_depth_index_all == nullptr) {
//     return false;
//   }
//   // 假设 max_depth_index_all 指针长度和 depth_map.size() 一致
//   if (map_info.depth_map[p.position].size() > 0) {
//     int idx = map_info.max_depth_index_all[p.position];
//     if (idx < 0 || idx >= map_info.depth_map[p.position].size()) {
//       return false;
//     }
//     const point_soph *max_point = map_info.depth_map[p.position][idx];
//     max_depth = max_point->vec(2);
//     float delta_t = (p.time - max_point->time);
//     depth_thr2_final = min(depth_thr2_final, v_min_thr2 * delta_t);
//   }
                                            ...
                                            bool interp)
{
  ...
  float cur_depth =
      max(cutoff_value, k_depth_max_thr2 * (p.vec(2) - d_depth_max_thr2)) +
      map_cons_depth_thr2;
  for (int ind_hor = -map_cons_hor_num2; ind_hor <= map_cons_hor_num2;
       ind_hor++)
  {
    for (int ind_ver = -map_cons_ver_num2; ind_ver <= map_cons_ver_num2;
         ind_ver++)
    {
      int pos_new = ((p.hor_ind + ind_hor) % MAX_1D) * MAX_1D_HALF +
                    ((p.ver_ind + ind_ver) % MAX_1D_HALF);
      ...
      {
        const point_soph *point = points_in_pixel[j];
        if (point->dyn == STATIC && fabs(p.time - point->time) > frame_dur &&
  ...
  if (interp && (p.local(0) < self_x_b || p.local(0) > self_x_f ||
                 p.local(1) > self_y_l || p.local(1) < self_y_r))
  {
    float cur_interp = interp_thr2 * (depth_map_list.back()->map_index -
                                      map_info.map_index + 1);
    float depth_all =
...
{
  if ((dataset == 0 && p_occ.is_distort) || (dataset == 0 && p.is_distort) ||
      p_occ.dyn == INVALID)
    return false;
                                              ...
                                              const DepthMap &map_info)
{
  ...
  for (int ind_hor = -depth_cons_hor_num2; ind_hor <= depth_cons_hor_num2;
       ind_hor++)
  {
    for (int ind_ver = -depth_cons_ver_num2; ind_ver <= depth_cons_ver_num2;
         ind_ver++)
    {
      int pos_new = ((p.hor_ind + ind_hor) % MAX_1D) * MAX_1D_HALF +
                    ((p.ver_ind + ind_ver) % MAX_1D_HALF);
      ...
      for (int j = 0; j < points_in_pixel.size(); j++)
      {
        const point_soph *point = points_in_pixel[j];
        if (fabs(point->time - p.time) < frame_dur &&
            fabs(point->vec(0) - p.vec(0)) < depth_cons_hor_thr2 &&
...
bool DynObjFilter::Case3(point_soph &p)
{
  ...
  {
    if (!Case3MapConsistencyCheck(p_spherical, *depth_map_list[first_i],
                                  case3_interp_en))
    {
      double ti = 0;
      float vi = 0;
      ...
      for (int ind_hor = -occ_hor_num3; ind_hor <= occ_hor_num3; ind_hor++)
      {
        for (int ind_ver = -occ_ver_num3; ind_ver <= occ_ver_num3; ind_ver++)
        {
          ...
          const vector<point_soph *> &points_in_pixel =
              depth_map_list[first_i]->depth_map[pos_new];
          if (depth_map_list[first_i]->max_depth_all[pos_new] <
              p_spherical.vec(2))
          {
            continue;
          }
                ...
                Case3DepthConsistencyCheck(*p_occ, *depth_map_list[first_i]))
            {
              ...
              while (i >= 0)
              {
                ...
                SphericalProjection(p, depth_map_list[i]->map_index,
                                    depth_map_list[i]->project_R,
                                    depth_map_list[i]->project_T, p);
                if (Case3MapConsistencyCheck(p, *depth_map_list[i],
                                             case3_interp_en))
                {
                  map_cons = false;
                  break;
...
bool DynObjFilter::Case3Enter(point_soph &p, const DepthMap &map_info)
{
  ...
  float depth_thr3_final =
      max(cutoff_value, k_depth_max_thr3 * (p.vec(2) - d_depth_max_thr3)) +
      occ_depth_thr3;
  if (map_info.depth_map[p.position].size() > 0)
  {
                                            ...
                                            bool interp)
{
  ...
  for (int ind_hor = -map_cons_hor_num3; ind_hor <= map_cons_hor_num3;
       ind_hor++)
  {
    for (int ind_ver = -map_cons_ver_num3; ind_ver <= map_cons_ver_num3;
         ind_ver++)
    {
      int pos_new = ((p.hor_ind + ind_hor) % MAX_1D) * MAX_1D_HALF +
                    ((p.ver_ind + ind_ver) % MAX_1D_HALF);
  ...
  if (interp && (p.local(0) < self_x_b || p.local(0) > self_x_f ||
                 p.local(1) > self_y_l || p.local(1) < self_y_r))
  {
    float cur_interp = interp_thr3 * (depth_map_list.back()->map_index -
                                      map_info.map_index + 1);
    float depth_all =
        DepthInterpolationAll(p, map_info.map_index, map_info.depth_map);
    if (fabs(p.vec(2) - depth_all) < cur_interp)
                                              ...
                                              const DepthMap &map_info)
{
  float all_minus = 0;
  int num = 0, smaller_num = 0, all_num = 0, greater_num = 0; //
  for (int ind_hor = -depth_cons_hor_num3; ind_hor <= depth_cons_hor_num3;
       ind_hor++)
  {
    for (int ind_ver = -depth_cons_ver_num3; ind_ver <= depth_cons_ver_num3;
         ind_ver++)
    {
      int pos_new = ((p.hor_ind + ind_hor) % MAX_1D) * MAX_1D_HALF +
                    ((p.ver_ind + ind_ver) % MAX_1D_HALF);
      ...
      for (int j = 0; j < points_in_pixel.size(); j++)
      {
        const point_soph *point = points_in_pixel[j];
        if (fabs(point->time - p.time) < frame_dur &&
            fabs(point->vec(0) - p.vec(0)) < depth_cons_hor_thr3 &&
...
void DynObjFilter::publish_dyn(std::string output_dir, std::string file_name)
{
  ...
  {
    cout << "Found Dynamic Objects, numbers: "
  ...
  {
    cout << "Found Dynamic Objects, numbers: "
         << laserCloudDynObj->points.size() << " Total time: " << time_total
         << " Average total time: " << time_total_avr << endl;
  }
  ...
  {
    cout << "Found Steady Objects, numbers: "
         << laserCloudSteadObj->points.size() << endl;
    if (laserCloudSteadObj_accu_times < laserCloudSteadObj_accu_limit)
    {
      laserCloudSteadObj_accu_times++;
