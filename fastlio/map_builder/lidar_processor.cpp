...
LidarProcessor::LidarProcessor(Config &config, std::shared_ptr<IESKF> kf) : m_config(config), m_kf(kf)
{
    ...
    m_cloud_down_world.reset(new CloudType(250000, 1));
    m_norm_vec.reset(new CloudType(250000, 1));
    m_effect_cloud_lidar.reset(new CloudType(250000, 1));
    m_effect_norm_vec.reset(new CloudType(250000, 1));
    m_nearest_points.resize(250000);
    m_point_selected_flag.resize(250000, false);
...
void LidarProcessor::incrCloudMap()
{
    ...
    std::cout << "[地图]incrCloudMap-> m_cloud_down_world size: " << m_cloud_down_world->size() << std::endl;
    printCloudRange(m_cloud_down_world, "m_cloud_down_world");
}
...
void LidarProcessor::process(SyncPackage &package)
{
    ...
    {
#if 0
        std::cout << "Downsampling point cloud with resolution: " << m_config.scan_resolution << std::endl;
        printCloudRange(package.cloud,"package.cloud");
        printCloudValid(package.cloud);
#endif
