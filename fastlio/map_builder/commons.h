...
{
    uint32_t seq = 0;
    double stamp = 0.0; // 或用 std::chrono::time_point
    std::string frame_id;
};
...
void printCloudRange(const CloudType::Ptr &cloud, const std::string &name, const std::string status = "OTHER");
void printCloudValid(const CloudType::Ptr &cloud);
void checkIMUVec(const std::vector<IMUData>& imus, const std::string& tag = "IMU");