...
bool IMUProcessor::initialize(SyncPackage &package)
{
    ...
    if (m_config.gravity_align)
    {
        ...
        m_kf->x().initGravityDir(acc_mean.normalized() * 9.81); 
        std::cout << "Gravity not aligned, gravity set to: " << m_kf->x().g.transpose() << std::endl;
    m_kf->P().setIdentity();
...
{
    // Check if the m_imu_cache has correct IMU data
    //std::cout << "[undistort] m_imu_cache.size() = " << m_imu_cache.size() << std::endl;
    m_imu_cache.clear();
    ...
    for (auto it_kp = m_poses_cache.end() - 1; it_kp != m_poses_cache.begin(); it_kp--)
    {
        ...
        for (; it_pcl->curvature > head->offset; it_pcl--)
        {
            dt = it_pcl->curvature - head->offset;
            dt *= double(1e-9); // 转换为秒
            V3D point(it_pcl->x, it_pcl->y, it_pcl->z);
            M3D expR = Sophus::SO3d::exp(imu_gyro * dt).matrix();
