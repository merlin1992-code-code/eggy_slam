...
void HBA::insert(pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud, const Pose &pose, const std::string& filename)
{
...
void HBA::writeJsonPoses(const std::string &path)
{
    json j;
    for (size_t i = 0; i < m_poses.size(); i++)
    {
        ...
        if (i < m_filenames.size() && !m_filenames[i].empty()) {
            file_name = m_filenames[i];
        } else {
            file_name = std::to_string(i) + ".pcd";
        }

        j[file_name]["pose"]["rotation"]["w"] = q.w();
        j[file_name]["pose"]["rotation"]["x"] = q.x();
        j[file_name]["pose"]["rotation"]["y"] = q.y();
        j[file_name]["pose"]["rotation"]["z"] = q.z();
        j[file_name]["pose"]["translation"]["x"] = t.x();
        j[file_name]["pose"]["translation"]["y"] = t.y();
        j[file_name]["pose"]["translation"]["z"] = t.z();
    }
    std::ofstream ofs(path);
    ofs << j.dump(4); // 缩进4格，便于阅读
    ofs.close();
}